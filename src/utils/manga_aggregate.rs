...
fn chap_hashmapentry_to_chapter_aggregate(input : (String, Vec<ApiObject<ChapterAttributes>>)) -> Result<ChapterAggregate>{
    let id = match input.1.get(0) {
        None => {
            return Err(std::io::Error::new(std::io::ErrorKind::NotFound, "The input vector is empty"));
        },
        Some(d) => d.clone()
    };
...
fn group_chapter_to_chapter_aggregate(input : Vec<ApiObject<ChapterAttributes>>) -> Result<Vec<ChapterAggregate>>{
...
fn group_chapter_to_volume_hash_map(input : Vec<ApiObject<ChapterAttributes>>) -> Result<ChapterVolumeHashMap>{
...
pub fn group_chapter_to_volume_aggregate(input : Vec<ApiObject<ChapterAttributes>>) -> Result<Vec<VolumeAggregate>>{
