...
pub fn initialise_data_dir() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(dirs_options.data_dir_add("history"))?;
...
pub fn verify_data_dir() -> anyhow::Result<String, String> {
    ...
    if !std::path::Path::new(dirs_options.data_dir_add("history").as_str()).exists() {
        return Err("the history dir doesn't exist".to_string());
    }
